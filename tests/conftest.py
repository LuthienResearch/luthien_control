...
def db_settings() -> Settings:
    ...
    try:
        ...
        assert all([settings.POSTGRES_USER, settings.POSTGRES_PASSWORD, settings.POSTGRES_HOST, settings.POSTGRES_PORT, settings.POSTGRES_DB])
        return settings
    except Exception as e:
        pytest.fail(f"Failed to load database settings via get_settings: {e}. Ensure POSTGRES_* env vars are set or available in .env")
