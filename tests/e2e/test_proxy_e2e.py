...
async def test_e2e_chat_completion(e2e_client: httpx.AsyncClient):
    """
    Performs an end-to-end test of the /v1/chat/completions endpoint.

    Sends a request through the proxy to the actual OpenAI backend (or configured backend)
    and verifies the basic structure of the response.
    """
    ...
    try:
        ...
        pytest.fail(f"E2E test failed with connection error: {e}")
    except Exception as e:
        pytest.fail(f"E2E test failed with unexpected error: {e}")


async def test_e2e_beta_chat_completion(e2e_client: httpx.AsyncClient):
    """
    Performs an end-to-end test of the /v1beta/chat/completions endpoint.

    Sends a request through the proxy (using the beta endpoint) to the actual
    OpenAI backend (or configured backend) and verifies the basic structure
    of the response.
    """
    print(f"\nTesting BETA against base URL: {e2e_client.base_url}")
    ...
    try:
        response = await e2e_client.post("/beta/chat/completions", json=request_payload)
        response.raise_for_status()  # Raise HTTPStatusError for 4xx/5xx responses
        ...
        print(
            f"BETA Chat completion E2E test PASSED. Response model: "
            f"{response_data.get('model')}, Finish reason: {choice.get('finish_reason')}"
        )

    except httpx.HTTPStatusError as e:
        pytest.fail(f"BETA E2E test failed with status {e.response.status_code}. Response: {e.response.text}")
    except httpx.RequestError as e:
        pytest.fail(f"BETA E2E test failed with connection error: {e}")
    except Exception as e:
        pytest.fail(f"BETA E2E test failed with unexpected error: {e}")
