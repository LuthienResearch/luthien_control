...
class DBLogger:
    ...
    def add_relationship(
        self, from_comm: Comm, to_comm: Comm, relationship_type: str, meta_info: Optional[Dict] = None
    ) -> CommRelationship:
    ...
    def get_related_comms(
        ...
        query = (
            self.session.query(Comm)
            .join(
                CommRelationship, (CommRelationship.to_comm_id == Comm.id) | (CommRelationship.from_comm_id == Comm.id)
            )
            .filter(
                ((CommRelationship.from_comm_id == comm.id) | (CommRelationship.to_comm_id == comm.id))
