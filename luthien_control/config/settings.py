...
class Settings(BaseSettings):
    ...
    def admin_dsn(self) -> str:
        ...
        """
        if not all([self.POSTGRES_USER, self.POSTGRES_PASSWORD, self.POSTGRES_HOST, self.POSTGRES_PORT]):
    ...
    def get_db_dsn(self, db_name: str | None = None) -> str:
        ...
        if not all([self.POSTGRES_USER, self.POSTGRES_PASSWORD, self.POSTGRES_HOST, self.POSTGRES_PORT]):
            raise ValueError("Missing required database settings (USER, PASSWORD, HOST, PORT) for get_db_dsn")
