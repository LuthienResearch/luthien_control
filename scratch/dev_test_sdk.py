...
def test_proxied_sdk():
    ...
    http_client = httpx.Client(transport=transport, verify=False, follow_redirects=True)

    # Configure OpenAI client with our custom HTTP client
    client = OpenAI(base_url=proxy_url, api_key=os.getenv("OPENAI_API_KEY"), http_client=http_client)
