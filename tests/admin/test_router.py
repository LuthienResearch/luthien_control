...
class TestLoginEndpoints:
    ...
    def test_login_success(self, admin_test_client, mock_csrf):
        ...
        with patch("luthien_control.admin.router.admin_auth_service.authenticate", AsyncMock(return_value=mock_user)):
            with patch(
                "luthien_control.admin.router.admin_auth_service.create_session", AsyncMock(return_value=mock_session)
            ):
                # Get CSRF token
...
class TestPolicyEdit:
    ...
    def test_update_policy_database_error(
        self, authenticated_admin_client, sample_policy, mock_csrf, mock_dependency_container
    ):
