...
def test_log_request_basic(api_logger, mock_log_handler):
    ...
    api_logger.log_request(method="GET", url="http://test.com/path", headers={"Accept": "*/*"})
...
def test_log_request_with_json_body(api_logger, mock_log_handler):
    ...
    api_logger.log_request(
        method="POST", url="http://test.com/data", headers={"Content-Type": "application/json"}, body=body_bytes
    )
    mock_log_handler.assert_called_once_with(
        {
            "type": "request",
...
def test_log_request_with_invalid_json_body(api_logger, mock_log_handler):
    ...
    api_logger.log_request(
        method="POST", url="http://test.com/invalid", headers={"Content-Type": "application/json"}, body=body_invalid
    )
    mock_log_handler.assert_called_once_with(
        {
            "type": "request",
...
def test_log_request_with_query_params(api_logger, mock_log_handler):
    ...
    api_logger.log_request(method="GET", url="http://test.com/search", headers={"Accept": "*/*"}, query_params=query)
    mock_log_handler.assert_called_once_with(
        {
            "type": "request",
            "method": "GET",
...
def test_log_request_header_redaction(api_logger, mock_log_handler):
    ...
    api_logger.log_request(method="GET", url="http://test.com/secure", headers=headers)
    mock_log_handler.assert_called_once_with(
        {
            "type": "request",
            "method": "GET",
...
def test_log_response_basic(api_logger, mock_log_handler):
    ...
    api_logger.log_response(status_code=204, headers={"Server": "TestServer"})
    mock_log_handler.assert_called_once_with(
        {"type": "response", "status_code": 204, "headers": {"Server": "TestServer"}, "timestamp": ANY}
    )
    # Check timestamp format
    call_args, _ = mock_log_handler.call_args
    assert isinstance(datetime.fromisoformat(call_args[0]["timestamp"]), datetime)
...
def test_log_response_with_json_body(api_logger, mock_log_handler):
    ...
    body_bytes = json.dumps(body_dict).encode("utf-8")
    api_logger.log_response(status_code=200, headers={"Content-Type": "application/json"}, body=body_bytes)
    mock_log_handler.assert_called_once_with(
        {
            "type": "response",
            "status_code": 200,
...
def test_log_response_with_text_body(api_logger, mock_log_handler):
    ...
    body_text = "Success"
    api_logger.log_response(status_code=200, headers={"Content-Type": "text/plain"}, body=body_text.encode("utf-8"))
    mock_log_handler.assert_called_once_with(
        {
            "type": "response",
            "status_code": 200,
...
def test_log_response_header_redaction(api_logger, mock_log_handler):
    ...
    api_logger.log_response(status_code=200, headers=headers, body=b'{"data": "test"}')
    mock_log_handler.assert_called_once_with(
        {
            "type": "response",
            "status_code": 200,
