...
TEST_ENV_VAR_NAME = "TEST_API_KEY_ENV"
...
class TestAddApiKeyHeaderFromEnvPolicyInit:
    ...
    def test_initialization_with_name(self):
        policy = AddApiKeyHeaderFromEnvPolicy(api_key_env_var_name=TEST_ENV_VAR_NAME, name="MyCustomPolicy")
        assert policy.name == "MyCustomPolicy"
        assert policy.api_key_env_var_name == TEST_ENV_VAR_NAME

    def test_initialization_without_name(self):
        policy = AddApiKeyHeaderFromEnvPolicy(api_key_env_var_name=TEST_ENV_VAR_NAME)
...
class TestAddApiKeyHeaderFromEnvPolicyApply:
    ...
    async def test_apply_success(
        ...
        monkeypatch.setenv(TEST_ENV_VAR_NAME, TEST_API_KEY)
        policy = AddApiKeyHeaderFromEnvPolicy(api_key_env_var_name=TEST_ENV_VAR_NAME)
        ...
        assert mock_transaction_context.request.headers["Authorization"] == f"Bearer {TEST_API_KEY}"
        assert mock_transaction_context.response is None  # No error response set
    ...
    async def test_apply_env_var_not_set_raises_api_key_not_found_error(
        ...
        monkeypatch.delenv(TEST_ENV_VAR_NAME, raising=False)
        policy = AddApiKeyHeaderFromEnvPolicy(api_key_env_var_name=TEST_ENV_VAR_NAME)

        expected_error_msg_fragment = (
            f"API key not found. Environment variable '{TEST_ENV_VAR_NAME}' is not set or is empty."
        )
        ...
        # Accessing private _body attribute for direct comparison of content
        assert (
    ...
    async def test_apply_env_var_set_to_empty_string_raises_api_key_not_found_error(
        ...
        monkeypatch.setenv(TEST_ENV_VAR_NAME, "")
        policy = AddApiKeyHeaderFromEnvPolicy(api_key_env_var_name=TEST_ENV_VAR_NAME)
...
class TestAddApiKeyHeaderFromEnvPolicySerialization:
    ...
    def test_serialize(self):
        policy = AddApiKeyHeaderFromEnvPolicy(api_key_env_var_name=TEST_ENV_VAR_NAME, name="TestSerialize")
        expected_config = {
            "name": "TestSerialize",
            "api_key_env_var_name": TEST_ENV_VAR_NAME,
        }
