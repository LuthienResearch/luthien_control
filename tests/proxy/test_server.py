...
from luthien_control.proxy.server import app, get_headers, config, policy_manager
...
async def test_proxy_request_success():
    ...
    # Patch dependencies
    with patch('luthien_control.proxy.server.policy_manager.apply_request_policies', 
               AsyncMock(return_value=mock_processed_request)), \
         patch('luthien_control.proxy.server.policy_manager.apply_response_policies',
...
async def test_proxy_request_with_content_encoding():
    ...
    with patch('luthien_control.proxy.server.policy_manager.apply_request_policies',
        ...
        mock_response = Mock(
            status_code=200,
            headers={'content-encoding': 'br, gzip'},
            aread=AsyncMock(return_value=b'compressed_data')
        )
