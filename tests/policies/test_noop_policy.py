...
async def test_process_request(mock_request):
    ...
    result = await policy.process_request(
        request=mock_request, target_url=test_url, headers=test_headers, body=test_body
    )
...
async def test_process_response(mock_request, mock_response):
    ...
    result = await policy.process_response(request=mock_request, response=mock_response, content=test_content)
...
async def test_process_response_with_empty_content(mock_request):
    ...
    result = await policy.process_response(request=mock_request, response=empty_response, content=test_content)
